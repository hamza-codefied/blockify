...
import { useCreateCustomGroup, useUpdateCustomGroup } from '@/hooks/useCustomGroups';
...
export const CustomGroupModal = ({ open, onClose, mode, groupData, onSuccess }) => {
  ...
  const shouldFetchStudents = open && !addEntireGrade && selectedGradeIds.length > 0;
    ...
    if (open) {
      if (isEditMode && groupData) {
        ...
        schedules.forEach(schedule => {
          ...
          formValues[`startTime_${dayName}`] = dayjs(schedule.startTime, 'HH:mm');
    ...
    try {
      ...
      if (addEntireGradeValue && (!gradeIds || gradeIds.length === 0)) {
        form.setFields([
          {
            name: 'gradeIds',
            errors: ['Please select at least one grade when "Add Entire Grade" is enabled'],
          },
        ]);
        return;
      }
      ...
      if (isEditMode && groupData?.id) {
        ...
        const schedulesChanged = 
          ...
          !schedules.every(s => {
            return originalData.schedules.some(os => 
        ...
        const payload = {
          ...
          studentIds: addEntireGradeValue ? undefined : (studentIds.length > 0 ? studentIds : undefined),
          addEntireGrade: addEntireGradeValue,
  ...
  const isLoading = isEditMode ? updateGroupMutation.isPending : createGroupMutation.isPending;
